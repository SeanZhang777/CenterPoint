...
def maxus_eval(gt_annos,
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            detail[curcls_name][f"bev@{min_overlaps[i, 1, j]:.2f}"] = mAPbev[j, :, i].tolist()
            detail[curcls_name][f"3d@{min_overlaps[i, 2, j]:.2f}"] = mAP3d[j, :, i].tolist()
            ...
            map_bev = ", ".join(f"{v:.2f}" for v in mAPbev[j, :, i])
            map_3d = ", ".join(f"{v:.2f}" for v in mAP3d[j, :, i])
            result += print_str(f"bev  AP:{map_bev}")
            result += print_str(f"3d   AP:{map_3d}")
            if compute_aos:
                detail[curcls_name][f"aos"] = mAPaos[j, :, i].tolist()
                map_aos = ", ".join(f"{v:.2f}" for v in mAPaos[j, :, i])
                result += print_str(f"aos  AP:{map_aos}")
    return {
        "result": result,
        "detail": detail,
    }
